(() => {
  ...
  // let's also map the pipes chars to ascii arrows chars
  ...
  // LET'S DRAW THE PATH
  // in INPUT, replace all the points in the PATH with a black square (█)
  // and all the other points not in the path with a white square (░)

  const DRAWN_PATH = INPUT.slice().map(row => row.map(() => '█')); // copy and fill with white squares

  for (let tile of path) {
    DRAWN_PATH[tile.y][tile.x] = '░';
  }

  console.log(DRAWN_PATH.map(row => row.join('')).join('\n'));
