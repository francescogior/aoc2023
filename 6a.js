(() => {
  ...
  const INPUT = INPUT_[0].map((n, i) => ({
    time: n,
    recordDistance: INPUT_[1][i],
  }));

  // console.log(INPUT);

  // Each item of the INPUT represents a race that lasts `time` milliseconds and has a record distance of `recordDistance`
  ...
  const result = INPUT.map(({ time, recordDistance }) => {
    let count = 0;
  ...
  console.log(result.reduce(multiply, 1));
